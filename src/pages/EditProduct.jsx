...
const EditProduct = () => {
  ...
  const fetchIdData = async (productId) => {
    try {
      ...
      if (responseData.success) {
        setData({
          ...data, // If you want to preserve previous state
          ...responseData.data, // Assuming actual product data is in `data`
        });
      } else {
  ...
  return (
    <div className="p-5">
      <MetaTags title={"Edit Product | Stock Management"} />
      ...
      <div className="bg-white mt-3 rounded">
        <div className="border-b flex gap-3 items-center p-2">
          <MdStore size={18} />
          <h2 className="font-semibold text-md">Edit Product</h2>
        </div>
        <div className="p-4">
          <form
            action=""

...
const OutStockDetails = () => {
  ...
  const exportCSV = () => {
    ...
    // Header row
    csvContent += "ID,Product Name,Unit,Quantity,Order Id,Date\n";
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      head: [["ID", "Product Name", "Unit", "Quantity", "Order Id", "Date"]],
        ...
        <table className="w-full border border-gray-300 mt-2 text-left bg-white">
          <thead>
            <tr className=" border-b">
              ...
              <th className="p-3 border-r">Order Id</th>
          ...
          <tbody>
            {data.map((row, index) => (
              <tr key={row._id} className="border-b hover:bg-gray-100">
                ...
                <td className="p-3 border-r">{row.orderId}</td>
